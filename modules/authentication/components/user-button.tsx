...
export default function UserButton({
  ...
  const formatMemberSince = (date?: string | Date | null) => {
    if (!date) return "";
  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Button
          variant="noShadow"
          className={`relative ${avatarSizes[size]} rounded-base p-0 overflow-hidden`}
          disabled={isLoading}
        >
          <Avatar className={`${avatarSizes[size]} rounded-base`}>
            <AvatarImage
              src={user.image || ""}
        ...
        <DropdownMenuLabel className="font-normal">
          <div className="flex flex-col space-y-2">
            <div className="flex items-center space-x-3">
              <Avatar className="h-12 w-12 border-2 border-border rounded-base">
                <AvatarImage
                  src={user.image || ""}
                  alt={user.name || "User avatar"}
                  className="rounded-base object-cover"
